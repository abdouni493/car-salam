  ...
  return (
    <div className="min-h-screen py-16 px-4 sm:px-6 lg:px-8" style={{ background: C.bg }}>
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        <div className="absolute top-0 right-0 w-[600px] h-[600px]" style={{ background: 'radial-gradient(circle, rgba(200, 16, 46, 0.04), transparent 70%)', transform: 'translate(30%, -30%)' }} />
        <div className="absolute bottom-0 left-0 w-[600px] h-[600px]" style={{ background: 'radial-gradient(circle, rgba(200, 16, 46, 0.05), transparent 70%)', transform: 'translate(-30%, 30%)' }} />
      </div>
                    ...
                    <motion.div
                      ...
                      style={{
                        background: isCompleted
                          ? 'linear-gradient(135deg, var(--color-vel-cta), var(--color-vel-cta-deep))'
                          : isCurrent
                          ? 'linear-gradient(135deg, rgba(200, 16, 46, 0.16), rgba(200, 16, 46, 0.18))'
                          : 'rgba(255, 255, 255, 0.05)',
                        border: isCompleted || isCurrent ? '2px solid var(--color-vel-cta)' : '2px solid rgba(255, 255, 255, 0.1)',
                        boxShadow: isCurrent ? '0 0 20px rgba(200, 16, 46, 0.25)' : 'none',
                        color: isCompleted ? C.black : isCurrent ? C.accent : 'var(--color-vel-dim)',
                      }}
                    >
                      {isCompleted ? <Check size={17} /> : <Icon size={17} />}
                    </motion.div>
                    <span className="text-[10px] font-bold tracking-wider uppercase hidden sm:block"
                      style={{ color: isCurrent || isCompleted ? C.accent : 'var(--color-vel-dim)', fontFamily: 'var(--font-display)' }}>
                      {meta.label[lang]}
                    </span>
                  </button>
                  {n < WIZARD_STEP_COUNT && (
                    <div className="flex-1 max-w-16 h-0.5 mx-1.5 sm:mx-3 mt-5 rounded-full transition-all duration-700"
                      style={{ background: step > n ? 'linear-gradient(90deg, var(--color-vel-cta), var(--color-vel-cta-deep))' : 'rgba(255, 255, 255, 0.06)' }}
                    />
                  )}
                </React.Fragment>
              );
